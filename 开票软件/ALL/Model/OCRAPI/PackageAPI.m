...
/*##################################################################################
 ...
 <json>返回数据json格式</json> 默认为0 xml格式  1为json格式
...
- (void)AFNuploadPackage:(NSData*)imageData Success:(SuccessBlock)sucess Fail:(ErrorBlock)fail {
    NSString *upAction    = @"doc.scan";
    NSString *username    = @"f78f1d67-f7c6-400e-a59e-d69cef650d55";
    NSString *psd         = @"fAhaDbznIwujdZHGvQbboLhNzNAwiw";
    ...
    upPackageString    = [[NSMutableString alloc] initWithFormat:@"<action>%@</action><client>%@</client><system>%@</system><password>%@</password><key>%@</key><time>%@</time><verify>%@</verify><ocrlang>%d</ocrlang><keylang>%d</keylang><ext>%@</ext><type>%@</type><file>%@</file><json>%@</json>", upAction, username, deviceType, md5Psd, rand, currentTime, verify,ocrlang, keylang,fileExt,@"1", imageStr,@"1"];

    NSMutableData *postData = (NSMutableData *)[upPackageString dataUsingEncoding:NSUTF8StringEncoding];
    [LoginModel httpManagerPostRestUrl:serverUrl Xml:postData Success:^(NSDictionary *dic, BOOL isSuccess) {
        
        
        sucess(dic,isSuccess);
    } Fail:^(NSError *error) {
         fail(error);
